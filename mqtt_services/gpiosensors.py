...
def main():
    ...
    mqttc = mqtt.Client()
    if args.url.username is not None:
        mqttc.username_pw_set(args.url.username, args.url.password)
    mqttc.on_connect = on_mqtt_connect
    mqttc.connect(args.url.hostname, args.url.port if args.url.port is not None else 1883, 60)
    mqttc.loop_start()
    ...
    for s in args.sensors:
        ...
        sensors.append(
            Sensor(
                {"name": d[0], "pin": int(d[1]), "pull_up": int(d[2]), "normal": int(d[3]), "status": 0, "changed": 0,
                 "alerted": 0, "timeout": int(d[4])}
            )
        )
