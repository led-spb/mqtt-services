...
class SSDPDiscovery(object):
    ...
    def start(self, st, period=10000, callback=None):
        ...
        self.ioloop.add_handler(self._sock.fileno(), self._handle_events,
                                tornado.ioloop.IOLoop.READ + tornado.ioloop.IOLoop.ERROR)

        self._discovery_loop()
        self._task = tornado.ioloop.PeriodicCallback(callback=self._discovery_loop, callback_time=period)
        self._task.start()
        pass

    def _discovery_loop(self):
        # logging.debug("Sending SSDP discovery packet")
        message = "\r\n".join([
            'M-SEARCH * HTTP/1.1',
...
class MyLGRemote(object):

    def __init__(self, client_key=None, ioloop=None, mqtt_broker=('localhost', 1883), auth=None):
        self.remote = LGRemote(client_key=client_key, ioloop=ioloop, on_close=self.on_close,
        ...
        self._mqttc = mqtt.TornadoMqttClient(ioloop, host=mqtt_broker[0], port=mqtt_broker[1])
        self._mqttc._client.on_message = self.on_mqtt_message
        self._mqttc._client.on_connect = self.on_mqtt_connect
        self._mqttc.start()
        pass
    ...
    def on_mqtt_connect(self, client, userdata, flags, rc):
        logging.info("MQTT connection: %s (%d)", mqtt.connack_string(rc), rc)
        if rc == 0:
            self._mqttc._client.subscribe("home/tv/control/#", 0)
            self.send_status()
        pass

    def on_mqtt_message(self, client, userdata, message):
        logging.debug("Got mqtt message: %s", message.topic)
    ...
    def send_status(self, changed=True):
        ...
        try:
            self._mqttc._client.publish("home/tv/detail", json.dumps(self.info), retain=True)
        except:
            logging.exception("Error while sending status to MQTT")
    ...
    def control_channel(self, channel):
        if self.info['status'] == 0:
            ...
            return
        channelId = None
        channel = unicode(channel, 'utf-8').lower()
